...
async def test_pygeoapi_service_hydrolocation(dbsession_containerized):
    ...
    assert float(actual[0].properties["measure"]) == pytest.approx(
        float(expected["features"][0]["properties"]["measure"]), abs=1e-2
    )
    ...
    assert actual[0].geometry["coordinates"][0] == pytest.approx(
        expected["features"][0]["geometry"]["coordinates"][0], abs=1e-2
    )
    assert actual[0].geometry["coordinates"][1] == pytest.approx(
        expected["features"][0]["geometry"]["coordinates"][1], abs=1e-2
    )
...
@pytest.mark.integration
async def test_api_get_root(f_client_containerized) -> None:
    r = await f_client_containerized.get(f"{API_PREFIX}?f=json")
...
@pytest.mark.integration
async def test_api_get_hydrolocation(f_client_containerized) -> None:
...
@pytest.mark.integration
async def test_api_get_basin_by_id(f_client_containerized) -> None:
    source_name = "wqp"
    identifier = "USGS-05427930"
    url = f"{API_PREFIX}/linked-data/{source_name}/{identifier}/basin?f=json"
    r = await f_client_containerized.get(url)
    assert r.status_code == 200
